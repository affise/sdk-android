    ...
    fun create(name: String?, map: Map<*, *>): Event? {
        ...
        return when (eventName) {
            ...
            EventName.AD_REVENUE ->
                AdRevenueEvent(userData = userData, timeStampMillis = timeStamp)

            EventName.ORDER_ITEM_ADDED ->
                OrderItemAddedEvent(userData = userData, timeStampMillis = timeStamp)

            EventName.ORDER_ITEM_REMOVE ->
                OrderItemRemoveEvent(userData = userData, timeStampMillis = timeStamp)
        }
    }
}