...
import com.affise.attribution.internal.InternalEventsParams
 ...
 */
internal class InternalEventsStorageImpl(
    ...
    override fun getEvents(key: String?): List<SerializedEvent> = getEventsDirectory(key)
        ...
        ?.filter { file ->
            //Filter old files
            (file.lastModified() > Calendar.getInstance().timeInMillis - InternalEventsParams.INTERNAL_EVENTS_STORE_TIME).also { isActual ->
                if (!isActual) {
                    //Delete old files
        ...
        ?.take(InternalEventsParams.INTERNAL_EVENTS_SEND_COUNT)
        ?.mapNotNull { file ->
            try {
                //Get data from file
     ...
     */
    override fun clear() {
        context.getDir(InternalEventsParams.INTERNAL_EVENTS_DIR_NAME, Context.MODE_PRIVATE).deleteRecursively()
    }
    ...
    private fun getEventsDirectory(key: String? = null): File {
        //Get root events dir
        val eventDir = context.getDir(InternalEventsParams.INTERNAL_EVENTS_DIR_NAME, Context.MODE_PRIVATE)
            .apply {
